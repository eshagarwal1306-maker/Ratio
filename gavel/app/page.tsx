      ...
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            style={{ backgroundColor: exportDone ? "#047857" : "#1c3461", transition: "background-color 0.3s" }}
          >
            {exporting ? <Spinner /> : null}
            {exporting ? "Exporting…" : exportDone ? "✓ Downloaded" : "Download Report (.docx)"}
          </button>
          <button
            onClick={handleEmail}
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold border transition-all"
            style={{
              borderColor: emailSent ? "#16a34a" : "#cbd5e1",
              color: emailSent ? "#16a34a" : "#374151",
              background: emailSent ? "#f0fdf4" : "transparent",
              transition: "all 0.3s",
            }}
          >
            {emailSent ? "✓ Email client opened" : "✉ Send by Email"}
          </button>
            ...
            {keptResults.map((result, i) => {
              ...
              return (
                <div key={i} className="px-7 py-5 space-y-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {result.claim.regulation && (
                          <a
                            href={getRegulationLink(result.claim.regulation, result.claim.article ?? "")}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-[10px] font-semibold text-blue-500 hover:underline hover:text-blue-700 transition-colors"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pl-16">
                    {[
                      {
                        label: "Citation",
                        verdict: result.sourcer.status,
                        detail: result.sourcer.discrepancy ?? result.sourcer.verbatim_quote,
                        link: getRegulationLink(result.claim.regulation, result.claim.article ?? ""),
                        linkLabel: "Verify →",
                      },
                      {
                        label: "Jurisdiction",
                        verdict: result.jurisdictionist.status,
                        detail: `${result.jurisdictionist.confidence}% confidence — ${result.jurisdictionist.reason}`,
                        link: null, linkLabel: null,
                      },
                      {
                        label: "Currency of law",
                        verdict: result.historian.status,
                        detail: result.historian.note,
                        link: getRegulationLink(result.claim.regulation, result.claim.article ?? ""),
                        linkLabel: "Check amendments →",
                      },
                      {
                        label: "Counter-arguments",
                        verdict: result.devils_advocate.strength,
                        detail: result.devils_advocate.counter_authority,
                        link: result.devils_advocate.counter_authority
                          ? `https://www.bailii.org/cgi-bin/bailey.cgi?Action=search&Query=${encodeURIComponent(result.devils_advocate.counter_authority)}`
                          : null,
                        linkLabel: "Search case law →",
                      },
                      {
                        label: "Conflict check",
                        verdict: result.firm_memory.status,
                        detail: result.firm_memory.reference ? `${result.firm_memory.reference.matter} (${result.firm_memory.reference.date})` : undefined,
                        link: null, linkLabel: null,
                      },
                    ].map((f) => (
                      <div key={f.label} className="flex items-start gap-2">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          {f.detail && (
                            <p className="text-[10px] text-slate-400 leading-snug">{f.detail}</p>
                          )}
                          {f.link && (
                            <a
                              href={f.link}
...
export default function HomePage() {
  ...
  const [citationExportDone, setCitationExportDone] = useState(false);
  const [citationEmailSent, setCitationEmailSent] = useState(false);
  ...
  async function exportCitationsDocx() {
    ...
    try {
      ...
      a.click();
      URL.revokeObjectURL(url);
      setCitationExportDone(true);
      setTimeout(() => setCitationExportDone(false), 2500);
    } finally {
  ...
  function emailCitations() {
    ...
    window.open(`mailto:?subject=${subject}&body=${body}`);
    setCitationEmailSent(true);
    setTimeout(() => setCitationEmailSent(false), 2500);
  }
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  background: citationEmailSent ? "#f0fdf4" : "transparent",
                  border: `1px solid ${citationEmailSent ? "#16a34a" : "#ddd9d1"}`,
                  fontSize: 12, fontWeight: 600,
                  color: citationEmailSent ? "#16a34a" : "#555",
                  transition: "all 0.3s",
                }}
              >
                {citationEmailSent ? "✓ Email client opened" : "✉ Send by email"}
              </button>
              <button
                ...
                style={{
                  ...
                  background: citationExportDone ? "#f0fdf4" : "transparent",
                  border: `1px solid ${citationExportDone ? "#16a34a" : "#ddd9d1"}`,
                  fontSize: 12, fontWeight: 600,
                  color: citationExportDone ? "#16a34a" : "#555",
                  opacity: exportingCitations ? 0.5 : 1,
                  transition: "all 0.3s",
                }}
              >
                {exportingCitations ? "Exporting…" : citationExportDone ? "✓ Downloaded" : "↓ Download .docx"}
              </button>
              <button
